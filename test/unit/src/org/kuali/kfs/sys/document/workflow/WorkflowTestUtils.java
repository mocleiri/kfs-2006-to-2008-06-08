...
public class WorkflowTestUtils {
    ...
    public static void waitForApproveRequest(Long docHeaderId, KualiUser user) throws Exception {
        LOG.info( "Entering: waitForApproveRequest("+docHeaderId+","+user.getUniversalUser().getPersonUserIdentifier()+")");
        ...
        Assert.assertTrue( "waitForApproveRequest("+docHeaderId+","+user.getUniversalUser().getPersonUserIdentifier()+") timed out", ChangeMonitor.waitUntilChange(monitor, 240, 5));
    }

...
<table cellpadding="0" cellspacing="0" class="datatable" summary=""> <tbody>
  ...
  <tr>
    ...
    <td class="infoline"><div align="center"> <html:image property="methodToCall.insertPeriodLine.anchor${currentTabIndex}" styleClass="tinybutton" src="${ConfigProperties.kr.externalizable.images.url}tinybutton-add1.gif" alt="add period line"/> </div></td>
  </tr>
  </c:if>
  ...
  <tr>
    ...
    <c:if test="${not viewOnly}"><td><div align="center"> <c:choose><c:when test="${ctr eq KualiForm.document.periodListSize-1}"> <html:image property="methodToCall.deletePeriodLine.anchorPeriods.line${ctr}" styleClass="tinybutton" src="${ConfigProperties.kr.externalizable.images.url}tinybutton-delete1.gif" alt="delete"/> </c:when><c:otherwise>&nbsp;</c:otherwise></c:choose> </div></td></c:if>
  </tr>
  </logic:iterate>
  <!-- End periods list. -->
</tbody></table>

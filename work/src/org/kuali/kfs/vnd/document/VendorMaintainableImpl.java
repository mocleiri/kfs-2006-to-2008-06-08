...
public class VendorMaintainableImpl extends KualiMaintainableImpl {
    ...
    public String getDocumentTitle(MaintenanceDocument document) {
        ...
        String specificTitle = SpringContext.getBean(KualiConfigurationService.class).getParameterValue(KFSConstants.PURAP_NAMESPACE,PurapParameterConstants.PURAP_OVERRIDE_VENDOR_DOC_TITLE);
    ...
    @Override
    /**
     *  This method is called for refreshing the vendorDetail. Currently we need this
     *  mainly for refreshing the soldToVendor object after returning from the lookup
     *  for a sold to vendor.
     */
     ...
     * 
     * This method will check whether the vendor has already had a vendor detail
     * assigned id. If not, it will call the private method to set the detail
     * assigned id. The method will also call the vendorService to determine whether
     * it should save the vendor header (i.e. if this is a parent) and will save the
     * vendor header accordingly. This is because we are not going to save vendor
     * header automatically along with the saving of vendor detail, so if the vendor
     * is a parent, we have to save the vendor header separately. 
     * 
     * Restriction-related information will be changed based on whether the Vendor Restricted
     * Indicator was changed.  If the Tax Number or Tax Type code have changed, the
     * fact will be recorded with a new record in the Tax Change table.
     * 
     * Finally the method will call the saveBusinessObject( ) of the super class
     * to save the vendor detail.
     */
    @Override
     ...
     * @see org.kuali.core.maintenance.Maintainable#saveBusinessObject()
     * 
     * If the vendorFirstLastNameIndicator is true, this method will set the vendor first name
     * and vendor last name fields from the vendorName field, then set the vendorName field
     * to null. Then it sets the businessObject of this maintainable to the VendorDetail
     * object that contains our modification to the name fields.
     * 
     */
    @Override
    ...
    /**
     * 
     * This method will set a valid detail assigned id to a vendor if the 
     * vendor has not had a detail assigned id yet. If this is a new parent 
     * whose header id is also null, this method will assign 0 as the detail 
     * assigned id. If this is a new division vendor, it will look for the count
     * of vendor details in the database whose vendor header id match with the
     * vendor header id of this new division, then look for the count of vendor
     * details in the database, in a while loop, to find if a vendor detail with
     * the same header id and detail id as the count has existed. If a vemdpr
     * with such criterias has existed, this method will increment the count
     * by 1 and look up in the database again. If it has not existed, assign
     * the count as the vendor detail id and change the boolean flag to stop
     * the loop, because we have already found the valid detail assigned id 
     * that we were looking for
     * 
     * @param vendorDetail VendorDetail The vendor whose detail assigned id
     ...
     * 
     * This method will be called if the user clicks on the "Create a new
     * division" link to create a new division vendor. By default, the
     * vendorParentIndicator is set to true in the constructor of VendorDetail,
     * but if we're creating a new division, it's not a parent, so we need
     * to set the vendorParentIndicator to false in this case.
     */
    @Override
     ...
     * 
     * This method is needed so that if the collection is the vendor supplier diversity and 
     * the vendor is not a parent, we'll not display the add line.
     */
    @Override

...
function clicker(e) {
	...
	if (target.href != null || hasValue(target.id, 'external') || isParentOrSelf(target, 'controls') || isParentOrSelf(target,'embed') || isParentOrSelf(target,'object')) {
return true;}
...
function externalLinks() {
	...
	for (var i=0; i<anchors.length; i++) {
		...
		if (anchor.getAttribute('href') && hasValue(anchor.id, 'external')) {
